...
func (m *Monitor) AddCheck(name string, interval time.Duration, fatal bool, config h.Config) {
	...
	if m.health == nil {
		...
		err := m.health.Stop()
		if err != nil {
			l.Warn("healthcheck.stop.error", "There was an issue stopping the health checks but it's not terminal")
		}
	}

	err := m.health.AddCheck(&config)
	if err != nil {
		l.Error("healthcheck.add.error", err)
	}

	err = m.health.Start()
	if err != nil {
		l.Error("healthcheck.start.error", err)
	}
}
